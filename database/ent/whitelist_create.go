...
func (wlc *WhiteListCreate) SetURL(s string) *WhiteListCreate {
	wlc.mutation.SetURL(s)
	return wlc
}
...
func (wlc *WhiteListCreate) SetOwner(p *Project) *WhiteListCreate {
	return wlc.SetOwnerID(p.ID)
}
...
func (wlc *WhiteListCreate) check() error {
	if _, ok := wlc.mutation.URL(); !ok {
		return &ValidationError{Name: "url", err: errors.New(`ent: missing required field "WhiteList.url"`)}
	}
	return nil
}
...
func (wlc *WhiteListCreate) createSpec() (*WhiteList, *sqlgraph.CreateSpec) {
	...
	if value, ok := wlc.mutation.URL(); ok {
	...
	if nodes := wlc.mutation.OwnerIDs(); len(nodes) > 0 {
		...
		_node.project_whitelists = &nodes[0]
